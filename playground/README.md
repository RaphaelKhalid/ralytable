...
`logos`-based lexer and the hand-written diagnostic renderer from
`compiler/crates/` are compiled to WebAssembly; the page analyses the buffer on
every keystroke and draws the result itself.
...
{
  "apiVersion": 1,
  "phases": { "lex": "ok", "parse": "not-implemented",
              "resolve": "not-implemented", "typecheck": "not-implemented" },
  "tokens": [
  ...
  "counts": { "errors": 1, "warnings": 0, "advice": 0, "tokens": 24, "lines": 7 },
...
- **`phases` is the honest part.** It reports `not-implemented` for everything
  after lexing, because that is the truth today. When the parser lands, `parse`
  flips to `"ok"`, an `ast` field appears next to `tokens`, syntax diagnostics
  join the existing `diagnostics` array, and nothing already in the shape
  changes. That is why the field is there from the start.

## What this demo does and does not prove

The lexer and the diagnostics are real, complete, and the same code the CLI
runs — the caret blocks on the page are byte-identical to
`raly check examples/broken.raly`.

There is no parser, no name resolution, and no type system. Nothing on the page
checks types or understands what a program means, and the page says so. The
example files are token soup by design: Raly has no grammar yet.
