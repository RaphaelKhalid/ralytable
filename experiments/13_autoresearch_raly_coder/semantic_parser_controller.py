"""Typed semantic-parser -> state-controller experiment."""
...
RULE_DIM = len(semantic.PREDICATES) + 2 * len(suite.CANDIDATES)
INPUT_DIM = semantic.STATE_DIM + RULE_DIM
...
def parse_rule(task: semantic.SemanticTask) -> ParsedRule:
    ...
    predicates = [
        (request.index(description), predicate)
        for predicate, description in semantic.PREDICATE_DESCRIPTIONS.items()
        if description in request
    ]
    actions = [
        (request.index(description), action)
        for action, description in semantic.ACTION_DESCRIPTIONS.items()
        if description in request
    ]
    if len(predicates) != 1 or len(actions) != 2:
        raise ValueError(f"parser ambiguity: {request}")
    actions.sort()
    return ParsedRule(predicates[0][1], actions[0][1], actions[1][1])


def rule_features(rule: ParsedRule) -> list[float]:
    result = [0.0] * RULE_DIM
    result[semantic.PREDICATES.index(rule.predicate)] = 1.0
    offset = len(semantic.PREDICATES)
    result[offset + suite.ACTION_TO_ID[rule.true_action]] = 1.0
    result[offset + len(suite.CANDIDATES) +
           suite.ACTION_TO_ID[rule.false_action]] = 1.0
    return result


def features(task: semantic.SemanticTask, corrupt: str | None = None) -> list[float]:
    return rule_features(parse_rule(task)) + semantic.state_features(task, corrupt)


class ParsedPolicy(nn.Module):
    def __init__(self):
        super().__init__()
...
def train(seed: int, tasks: list[semantic.SemanticTask], updates: int,
          device: torch.device) -> tuple[ParsedPolicy, list[float]]:
    torch.manual_seed(seed)
    model = ParsedPolicy().to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=3e-3)
    losses: list[float] = []
    for update in range(updates):
        batch = [tasks[(update * 7 + index * 17) % len(tasks)]
                 for index in range(min(64, len(tasks)))]
        inputs = torch.tensor([features(task) for task in batch],
...
def prefix_values(task: semantic.SemanticTask,
                  corrupt: str | None = None) -> tuple[int, ...]:
...
@torch.no_grad()
def learned_action(model: ParsedPolicy, task: semantic.SemanticTask,
...
def learned_public_action(model: ParsedPolicy, task: semantic.SemanticTask,
...
def causal(model: ParsedPolicy, tasks: list[semantic.SemanticTask],
           device: torch.device) -> dict[str, float]:
    changed, preserved = [], []
    for task in tasks:
        base = learned_action(model, task, device)
        altered = learned_action(model, task, device, "erase_value")
        placebo = learned_action(model, task, device, "noise")
        changed.append(float(altered != base))
        preserved.append(float(placebo == base))
    return {
        "parsed_state_relevant_changed_rate": statistics.mean(changed),
        "parsed_state_irrelevant_preserved_rate": statistics.mean(preserved),
        "parsed_state_causal_rate": statistics.mean(
            float(a and b) for a, b in zip(changed, preserved)
        ),
    }


def metrics(tasks: list[semantic.SemanticTask], model: ParsedPolicy | None,
    ...
    for task in tasks:
        ...
        if direction == "parsed-symbolic":
            chosen, expanded = raw_action, 0
        elif direction == "parsed-learned-public":
    ...
    row: dict[str, object] = {
        ...
        "parsed_state_relevant_changed_rate": 0.0,
        "parsed_state_irrelevant_preserved_rate": 0.0,
        "parsed_state_causal_rate": 0.0,
    }
...
def main() -> None:
    ...
    for seed_text in args.seeds.split(","):
        seed = int(seed_text)
        train_tasks = semantic.make_dataset(random.Random(610000 + seed), args.train_count)
        eval_tasks = semantic.make_dataset(random.Random(620000), args.eval_count)
        for direction, model_mode in (("parsed-symbolic", None),
                                      ("parsed-learned", "learned"),
                                      ("parsed-learned-public", "learned")):
            ...
            row.update({
                ...
                "checkpoint": f"parsed-seed-{seed}-u{args.updates}-{direction}",
                "hypothesis": "typed semantic parsing should isolate language from state control",
